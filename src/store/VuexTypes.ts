/**
 * 实验室
 */
export const LIST_LABS = "LIST_LABS";
export const POST_LAB = "POST_LAB";
export const UPDATE_LAB = "UPDATE_LAB";
export const DELETE_LAB = "DELETE_LAB";
export const GET_LABS_BY_COURSE_ID = "GET_LABS_BY_COURSE_ID";
/**
 * 教师
 */
export const LIST_USERS = "LIST_USERS";
export const POST_USER = "POST_USER";
export const UPDATE_USER = "UPDATE_USER";
export const DELETE_USER = "DELETE_USER";
/**
 * 课程
 */
 export const LIST_COURSES = "LIST_COURSES";
 export const POST_COURSE = "POST_COURSE";
 export const UPDATE_COURSE = "UPDATE_COURSE";
 export const DELETE_COURSE = "DELETE_COURSE";
 export const GET_COURSE_BY_ID = "GET_COURSE_BY_ID";
 export const GET_ALL_COURSES = "GET_ALL_COURSES";
 /**
 * 预约
 */
  export const GET_APPOINTMENTS_BY_LAB_NAME = "GET_APPOINTMENTS_BY_LAB_NAME";
  export const POST_APPOINTMENT = "POST_APPOINTMENT";
  export const GET_SURPLUS_PERIODS = "GET_SURPLUS_PERIODS";
